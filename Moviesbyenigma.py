...
def obtener_ruta_almacenamiento():
    """
    Detecta el entorno y asegura que el archivo se guarde 
    directamente en la memoria interna del celular (Carpeta Downloads).
    """
    rutas_probables = [
        "/sdcard/Download",
...
def descargar_archivo_real(url_origen, nombre_archivo):
    """
    Descarga usando urllib nativo escribiendo directo a la memoria pública.
    """
    ...
    try:
        req = urllib.request.Request(url_origen, headers=cabeceras)
        with urllib.request.urlopen(req, timeout=30) as respuesta:
            # Línea 83 corregida y blindada en una sola tira de texto limpia
            ...
            with open(ruta_completa, 'wb') as f:
                while True:
                    chunk = respuesta.read(block_size)
                    if not chunk:
                        break
                    f.write(chunk)
                    ...
                    if tamanio_total > 0:
                        ...
                        print(f"\r🚀 Descargando: {porcentaje}% [{descargado // 1024 // 1024} MB / {tamanio_total // 1024 // 1024} MB] | ByEnigma2.0", end="")
                    else:
                        print(f"\r🚀 Descargando: {descargado // 1024 // 1024} MB recibidos... | ByEnigma2.0", end="")
                        
        print(f"\n\n✅ ¡Operación Exitosa por ByEnigma2.0!")
        print(f"🎬 Video completo guardado en tu memoria interna.")
        
    except Exception as e:
        print(f"\n❌ Falla de escritura o red: {e}")
...
def mostrar_menu_interno_peliculas(nombre_seccion, lista_pelis):
    ...
    while True:
        ...
        if opcion == 'R':
            break
        elif opcion == 'N' and 'N' in unicode(controles) if sys.version_info[0] < 3 else 'N' in controles:
            if 'N' in controles: pagina_actual += 1
...
def iniciar_bunker():
    ...
    try:
        ...
        req = urllib.request.Request(enlace_iptv, headers=cabeceras)
        
        with urllib.request.urlopen(req, timeout=25) as respuesta:
            contenido_texto = respuesta.read().decode('utf-8', errors='ignore')
        ...
        print(f"❌ Fallo en la conexión nativa: {e}")
